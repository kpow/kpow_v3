...
export function VenueMap({ venues, onVenueSelect }: VenueMapProps) {
  ...
  // Create a unique handler for marker clicks
  const handleVenueMarkerClick = (venueId: string) => (e: L.LeafletMouseEvent) => {
    if (e.originalEvent) {
      e.originalEvent.stopImmediatePropagation();
      e.originalEvent.preventDefault();
    }
    if (onVenueSelect) {
      setTimeout(() => onVenueSelect(venueId), 0);
    }
  };
          ...
          >
            ...
            {validVenues.map((venue) => (
              <Marker
                ...
                icon={defaultIcon}
                eventHandlers={{
                  click: handleVenueMarkerClick(venue.venue)
                }}
              >
                <Popup>
                  <div>
