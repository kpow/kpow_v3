...
export function SongDataTable({ onArtistClick }: SongDataTableProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ["/api/music/songs", page, sorting],
    queryFn: async () => {
      const response = await fetch(`/api/music/songs?page=${page}&sortBy=${sorting[0]?.id || ''}&sortOrder=${sorting[0]?.desc ? 'desc' : 'asc'}`);
